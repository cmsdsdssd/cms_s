...
-- ------------------------------------------------------------
-- cms_0255: fix confirm_shipment schema mismatch
--
-- Fixes runtime error:
--   code 42703 / message: record "r_order" has no field "category_code"
-- Root cause:
--   public.cms_order_line table (cms_0002_tables.sql) does not have category_code.
--   Some deployed versions of cms_fn_confirm_shipment referenced r_order.category_code.
--
-- Additional fix:
--   Avoid referencing non-existent record field r_master.material (use v_material).
--
-- This migration overwrites public.cms_fn_confirm_shipment(uuid,uuid,text) with the repo's latest
-- implementation (cms_0248) plus the above fixes.
-- ------------------------------------------------------------
...
begin
  ...
  from public.cms_fn_latest_tick_by_role_v1('GOLD')
  order by observed_at desc
  limit 1;

  select t.tick_id, t.price, t.meta
  into v_silver_tick_id, v_silver_price, v_silver_tick_meta
  from public.cms_fn_latest_tick_by_role_v1('SILVER_KR')
  order by observed_at desc
  limit 1;
  ...
  loop
    ...
    v_is_plated := false;
    v_plating_amount_sell := 0;
    v_plating_amount_cost := 0;
...
-- category resolve
    ...
    -- material sell/cost (simplified: keep existing behavior)
