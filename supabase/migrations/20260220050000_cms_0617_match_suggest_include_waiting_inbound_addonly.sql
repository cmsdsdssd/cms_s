-- cms_0617 (ADD-ONLY)
-- 매칭 후보 조회 시 WAITING_INBOUND 상태도 포함
-- 이유: 매칭 취소 시 order_line이 WAITING_INBOUND로 변경되는데,
--       match_suggest는 SENT_TO_VENDOR만 찾아서 후보가 없다고 표시됨
...
begin
  ...
  with orders as (
    ...
    where (
      ...
      and ol.status in ('SENT_TO_VENDOR'::public.cms_e_order_status, 'WAITING_INBOUND'::public.cms_e_order_status)
