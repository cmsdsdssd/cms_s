-- 🔥🔥🔥 근본 해결: 출고 확정 시 수동 공임이 계산에 반영되도록 수정
...
-- 1. 기존 함수 드롭 (의존성 있는 함수들도 함께 드롭 후 재생성)
...
SELECT 
  '함수 수정 완료' as 결과,
  proname as 함수명,
  pg_get_function_arguments(oid) as 매개변수
FROM pg_proc
WHERE proname IN ('cms_fn_confirm_shipment', 'cms_fn_confirm_shipment_v2', 'cms_fn_confirm_shipment_v3_cost_v1')
ORDER BY proname;
