...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'cms_vendor_fax_config' AND policyname = 'allow_anon_select_fax_config'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'cms_vendor_fax_config' AND policyname = 'allow_auth_upsert_fax_config'
    ) THEN
