-- cms_0400: AR payment FIFO v2
-- policy:
-- 1) cash always allocates labor first across all invoices (FIFO)
-- 2) material cash allocation is optional via p_allow_cash_for_material (default OFF)
-- 3) when material cash is allocated for commodity invoices, commodity grams are reduced synchronously

create or replace function public.cms_fn_ar_apply_payment_fifo_v2(
  p_party_id uuid,
...
begin
  ...
  -- Cash pass #1: labor only (global FIFO)
  for r_invoice in
  ...
  -- Cash pass #2: material (optional, global FIFO)
