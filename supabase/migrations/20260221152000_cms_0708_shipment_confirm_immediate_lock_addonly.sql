-- cms_0708: shipment-confirm immediate lock (add-only)
-- Policy: once shipment is confirmed, line pricing/weights and AR principal snapshots are immutable.
...
begin
  ...
  if found then
    if abs(v_existing_amount - v_total_sell) > 0.000001
      ...
      raise exception 'shipment ledger immutable after confirm (shipment_id=%)', p_shipment_id;
    end if;
...
declare
  ...
  v_has_shipment_ledger boolean := false;
begin
  select exists (
    select 1
  ...
  select exists (
    ...
    where l.entry_type = 'SHIPMENT'
      and l.shipment_id = p_shipment_id
  ) into v_has_shipment_ledger;

  if v_has_shipment_ledger then
    raise exception 'cannot re-apply material factor snapshot after shipment confirm lock (shipment_id=%)', p_shipment_id;
  end if;
...
begin
  ...
  return jsonb_build_object(
    ...
    'updated', 0,
    'inserted', v_inserted,
    'locked_after_confirm', v_existing_count > 0
  );
end $$;
...
begin
  ...
  if v_hdr.confirmed_at is not null or v_hdr.status = 'CONFIRMED'::public.cms_e_shipment_status then
  ...
  if v_measured is not null and v_deduct > v_measured then raise exception 'deduction_weight_g cannot exceed measured_weight_g'; end if;
...
as $$
begin
  raise exception 'unconfirm is disabled: shipment is immutable after confirm (shipment_id=%)', p_shipment_id;
end $$;
