-- Add advisory lock to prevent concurrent duplicate shipment creation
-- This prevents race conditions when the same order_line_id is processed simultaneously
...
declare
  ...
  v_existing record;
begin
  -- ✅ NEW: Advisory lock to prevent concurrent duplicate creation
  -- Lock by order_line_id to serialize upsert operations for the same order
