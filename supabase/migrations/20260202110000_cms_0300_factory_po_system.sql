-- Migration: Factory Purchase Order (공장발주) System
-- Date: 2026-02-02
-- ADD-ONLY: Only adds new columns/tables, does not modify existing columns
-- FIXED: 테이블 생성 순서 재배열 (참조되는 테이블 먼저 생성)

-- ============================================
-- 1. Create Factory Purchase Order (PO) table FIRST
--    (이 테이블은 cms_order_line이 참조하므로 먼저 생성)
-- ============================================
CREATE TABLE IF NOT EXISTS cms_factory_po (
    ...
    fax_payload_url text, -- PDF storage path
    fax_provider_message_id text, -- Provider's message ID for tracking
...
CREATE TABLE IF NOT EXISTS cms_vendor_fax_config (
    ...
    sender_phone text,
    sender_fax text,
    cover_page_template text, -- Template for cover page
...
DO $$
BEGIN
    -- Add factory_po_id column if not exists
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'cms_order_line' AND column_name = 'factory_po_id'
    ) THEN
        ALTER TABLE cms_order_line ADD COLUMN factory_po_id uuid REFERENCES cms_factory_po(po_id);
    END IF;

    -- Add sent_to_vendor_at timestamp
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'cms_order_line' AND column_name = 'sent_to_vendor_at'
    ) THEN
        ALTER TABLE cms_order_line ADD COLUMN sent_to_vendor_at timestamptz;
    END IF;

    -- Add inbound_at timestamp (receipt based snapshot)
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'cms_order_line' AND column_name = 'inbound_at'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'cms_order_line' AND column_name = 'shipped_at'
    ) THEN
        ALTER TABLE cms_order_line ADD COLUMN shipped_at timestamptz;
    END IF;

    -- Add vendor_prefix column (extracted from model_name)
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'cms_order_line' AND column_name = 'vendor_prefix'
    ) THEN
...
SELECT 
    ...
    -- Calculate estimated inbound status based on receipt
    CASE 
        WHEN ol.inbound_at IS NOT NULL THEN true
        ELSE false
    END AS has_inbound_receipt,
...
BEGIN
    ...
    IF NEW.model_name IS NOT NULL THEN
        ...
        -- Only set if not empty and looks like a factory code (alphanumeric)
...
-- 10. Grants
-- ============================================
-- Note: uuid with gen_random_uuid() default does not create sequences
