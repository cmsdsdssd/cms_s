-- cms_0707: lock shipment/AR recompute after payment allocation and align single-factor semantics
...
begin
  ...
  select exists (
    ...
    join public.cms_ar_payment_alloc pa
      on pa.ar_id = ai.ar_id
  ...
  if v_has_allocated_payment then
    ...
    if not found then
      raise exception 'cannot sync AR ledger for paid shipment without existing shipment ledger row (shipment_id=%)', p_shipment_id;
    end if;
    ...
    if abs(v_existing_amount - v_total_sell) > 0.000001
      ...
      raise exception 'cannot mutate shipment ledger after payment allocation (shipment_id=%)', p_shipment_id;
    end if;
