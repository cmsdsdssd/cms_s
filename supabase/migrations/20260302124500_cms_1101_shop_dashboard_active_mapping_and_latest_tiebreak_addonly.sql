...
-- -----------------------------------------------------------------------------
-- cms_1101_shop_dashboard_active_mapping_and_latest_tiebreak_addonly
-- 1) dashboard view uses active channel mappings only
-- 2) latest views get deterministic tie-break with PK
-- -----------------------------------------------------------------------------
  ...
  select
    ...
    row_number() over (
      partition by ps.channel_id,
                   ...
                   ps.master_item_id
      order by ps.computed_at desc, ps.created_at desc, ps.snapshot_id desc
    ) as rn
...
with ranked as (
  select
    ...
    row_number() over (
      ...
      order by cps.fetched_at desc, cps.created_at desc, cps.channel_price_snapshot_id desc
    ) as rn
