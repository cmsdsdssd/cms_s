...
do $$
begin
  -- Explicit signature (avoid ambiguity / overload errors)
...
begin
  ...
  v_center_supply := case
    ...
    else coalesce(v_order.center_stone_source, 'SELF'::public.cms_e_stone_supply_source)
  end;
  v_sub1_supply := case
    when v_order.sub1_stone_name is null then null
    else coalesce(v_order.sub1_stone_source, 'SELF'::public.cms_e_stone_supply_source)
  end;
  v_sub2_supply := case
    when v_order.sub2_stone_name is null then null
    else coalesce(v_order.sub2_stone_source, 'SELF'::public.cms_e_stone_supply_source)
  end;

  -- self stone cost only
  v_stone_cost_total_self :=
    (case when v_center_supply = 'SELF'::public.cms_e_stone_supply_source then greatest(v_center_qty,0) * greatest(v_center_unit_cost,0) else 0 end)
    + (case when v_sub1_supply   = 'SELF'::public.cms_e_stone_supply_source then greatest(v_sub1_qty,0)   * greatest(v_sub1_unit_cost,0)   else 0 end)
    + (case when v_sub2_supply   = 'SELF'::public.cms_e_stone_supply_source then greatest(v_sub2_qty,0)   * greatest(v_sub2_unit_cost,0)   else 0 end);
