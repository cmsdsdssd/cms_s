...
begin
  ...
  if v_order.matched_master_id is null then
    raise exception 'P0001: Order must be matched to a master item before shipping (Order: %, Item: %)', v_order.order_no, v_order.model_name;
  end if;
  ...
  if not found then
    raise exception 'P0002: Matched master item not found (ID: %)', v_order.matched_master_id;
  end if;
  ...
  if v_category_code is null then
    raise exception 'category_code is null in master_item (ID: %)', v_master.master_id;
  end if;
