...
-- -----------------------------------------------------------------------------
-- cms_1015_shop_dashboard_snapshot_join_fix_addonly
-- Fix wrong target-price binding by making pricing latest and dashboard joins
-- channel_product-aware (not master-only).
-- -----------------------------------------------------------------------------

create index if not exists idx_pricing_snapshot_latest_cp_rank
  on public.pricing_snapshot(channel_id, channel_product_id, master_item_id, computed_at desc, created_at desc);
...
left join ps
 ...
 and ps.channel_product_id = m.channel_product_id
