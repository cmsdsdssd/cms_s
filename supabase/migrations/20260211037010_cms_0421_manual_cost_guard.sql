-- Add MANUAL cost validation guards
-- This migration adds validation to prevent invalid costs in MANUAL mode:
-- - Empty cost_lines array
-- - unit_cost_krw <= 0
-- - Missing lines (not all shipment lines have costs)
...
declare
  ...
  v_shipment_line_count int;
  v_cost_lines_count int;
...
begin
  ...
  -- ✅ NEW: MANUAL mode validation guards
  if v_mode = 'MANUAL' then
    -- Check 1: cost_lines must not be empty
    if jsonb_array_length(coalesce(p_cost_lines,'[]'::jsonb)) = 0 then
      raise exception using errcode='P0001',
        message='MANUAL mode: cost_lines cannot be empty';
    end if;

    -- Check 2: all unit_cost_krw must be > 0
    select count(*)
    into v_invalid_cost_count
    from jsonb_array_elements(p_cost_lines) e
    where coalesce(nullif((e->>'unit_cost_krw')::text,'')::numeric, 0) <= 0;

    if v_invalid_cost_count > 0 then
      raise exception using errcode='P0001',
        message=format('MANUAL mode: all unit_cost_krw values must be > 0 (found %s invalid)', v_invalid_cost_count);
    end if;

    -- Check 3: all shipment lines must have costs provided
    select count(*) into v_shipment_line_count
    from public.cms_shipment_line
    where shipment_id = p_shipment_id;

    select count(distinct nullif((e->>'shipment_line_id')::text,'')::uuid)
    into v_cost_lines_count
    from jsonb_array_elements(p_cost_lines) e
    where nullif((e->>'shipment_line_id')::text,'')::uuid is not null;

    if v_cost_lines_count < v_shipment_line_count then
      raise exception using errcode='P0001',
        message=format('MANUAL mode: all shipment lines must have costs (expected %s, got %s)',
          v_shipment_line_count, v_cost_lines_count);
    end if;
  end if;
