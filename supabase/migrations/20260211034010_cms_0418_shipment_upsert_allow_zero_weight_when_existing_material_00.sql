...
begin
  ...
  -- check existing shipment line first (material may already be edited to '00')
  ...
  -- allow-zero decision: order material OR master default OR existing draft material
  v_material := v_order.material_code;
  if v_material is null and v_order.matched_master_id is not null then
    select m.material_code_default
      into v_material
