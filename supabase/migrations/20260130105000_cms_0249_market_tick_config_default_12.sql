...
-- Ensure DEFAULT row exists (insert only if missing)
insert into public.cms_market_tick_config (config_key, cs_correction_factor, updated_at)
select 'DEFAULT', 1.200000, now()
where not exists (
  select 1 from public.cms_market_tick_config where config_key = 'DEFAULT'
);
