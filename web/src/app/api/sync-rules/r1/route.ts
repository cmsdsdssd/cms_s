...
const validateR1Policy = (sourceMaterialCode: string | null, targetMaterialCode: string): string | null => {
  if (sourceMaterialCode !== "14" || targetMaterialCode !== "18") {
    return "R1 policy allows only source_material_code=14 and target_material_code=18";
  }
  return null;
};
...
export async function POST(request: Request) {
  ...
  if (process.env.ENABLE_R1_LINKAGE !== "true") {
    return jsonError("R1 material linkage is disabled. Manage pricing by source-specific R2 margins.", 409);
  }
  ...
  const policyError = validateR1Policy(sourceMaterialCode, targetMaterialCode);
...
export async function PUT(request: Request) {
  ...
  if (error) return jsonError(error.message ?? "R1 룰 수정 실패", 400);
...
export async function DELETE(request: Request) {
  ...
  const [unlinkR2Res, unlinkR4Res] = await Promise.all([
    sb
      .from("sync_rule_r2_size_weight")
      .update({ linked_r1_rule_id: null })
      .eq("linked_r1_rule_id", ruleId),
    sb
      .from("sync_rule_r4_decoration")
      .update({ linked_r1_rule_id: null })
      .eq("linked_r1_rule_id", ruleId),
  ]);
  if (unlinkR2Res.error) return jsonError(unlinkR2Res.error.message ?? "R1 연동 R2 정리 실패", 500);
  if (unlinkR4Res.error) return jsonError(unlinkR4Res.error.message ?? "R1 연동 R4 정리 실패", 500);
  ...
  const { data, error } = await sb
    ...
    .select("rule_id")
    .maybeSingle();
  if (error) return jsonError(error.message ?? "R1 룰 삭제 실패", 400);
  if (!data) return jsonError("rule_id not found", 404);
