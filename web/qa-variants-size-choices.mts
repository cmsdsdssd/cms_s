...
console.log(JSON.stringify({
  sizes_by_material: json?.data?.option_detail_allowlist?.sizes_by_material,
  canonical_size_rows: json?.data?.canonical_option_rows?.filter((row: any) => row.category_key === 'SIZE').slice(0, 5),
}, null, 2));
