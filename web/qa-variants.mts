...
console.log(JSON.stringify({
  saved_option_categories: json?.data?.saved_option_categories,
  canonical_option_rows: json?.data?.canonical_option_rows?.filter((row: any) => row.category_key === 'SIZE' || row.category_key === 'COLOR_PLATING').slice(0, 12),
}, null, 2));
