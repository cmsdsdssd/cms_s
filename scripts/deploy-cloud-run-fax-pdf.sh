...
gcloud run deploy "${SERVICE_NAME}" \
  ...
  --timeout 600 \
  --port 8080 \
  --update-env-vars "TZ=Asia/Seoul,SUPABASE_URL=${NEXT_PUBLIC_SUPABASE_URL_VALUE},NEXT_PUBLIC_SUPABASE_URL=${NEXT_PUBLIC_SUPABASE_URL_VALUE},NEXT_PUBLIC_SUPABASE_ANON_KEY=${NEXT_PUBLIC_SUPABASE_ANON_KEY_VALUE},SHOP_SYNC_DEFAULT_INTERVAL_MINUTES=5,SHOP_SYNC_FORCE_INTERVAL_MINUTES=5" \
