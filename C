...
export default function ShipmentsMainPage() {
  ...
  const debugOrderLineId = useMemo(() => {
    if (typeof window === "undefined") return "";
    const params = new URLSearchParams(window.location.search);
    return params.get("debugOrderLineId") ?? "";
  }, []);

  const debugMode = useMemo(() => {
    if (typeof window === "undefined") return false;
    return new URLSearchParams(window.location.search).get("debugShipments") === "1";
  }, []);

  const debugOrderLineId = useMemo(() => {
    if (typeof window === "undefined") return "";
    return new URLSearchParams(window.location.search).get("debugOrderLineId") ?? "";
  }, []);
  ...
  const storePickupOrderLineIds = useMemo(() => {
    ...
    return new Set(rows.map((row) => row.order_line_id ?? "").filter((value) => Boolean(value)));
  ...
  const applyFilters = useMemo(() => {
    ...
    const matchesFilter = (row: UnshippedRow, filter: FilterRow) => {
      ...
      if (filter.type === "date") {
        const dates = [row.queue_sort_date, row.sent_to_vendor_at, row.inbound_at].filter(Boolean);
  ...
  const debugSnapshot = useMemo(() => {
    const orderLineId = debugOrderLineId.trim();
    const unshippedRows = unshippedQuery.data ?? [];
    const inUnshipped = orderLineId
      ? unshippedRows.find((row) => row.order_line_id === orderLineId) ?? null
      : null;
    const hasStorePickup = orderLineId ? storePickupOrderLineIds.has(orderLineId) : false;
    const activeFilters = filters.filter((filter) => filter.value);
    ...
    return {
      ...
      inUnshipped,
      storePickupCount: storePickupOrderLineIds.size,
      hasStorePickup,
      filteredCount: applyFilters.length,
      page,
      totalPages,
  ...
  useEffect(() => {
    ...
    console.log("[shipments_main][debug]", {
      includeStorePickup,
      filterOperator,
      filters,
      activeFilters,
  ...
  if (error) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-96 gap-4 px-4">
        ...
        <div className="text-sm text-gray-600 max-w-md text-center">{errorMessage}</div>
        {isMigrationError && (
          <div className="text-xs text-amber-600 bg-amber-50 p-3 rounded-lg max-w-md">
            <strong>해결 방법:</strong>
            <br />
            1. 터미널에서 <code className="bg-gray-200 px-1">npx supabase db reset</code> 실행
            <br />
            2. 페이지 새로고침 (F5)
          </div>
        )}
        <div className="flex gap-2">
          <Button onClick={() => unshippedQuery.refetch()} variant="secondary">
            재시도
          </Button>
  ...
  return (
    <div className="space-y-3" id="shipments_main.root">
      <div className="mx-4 rounded-lg border-2 border-red-500 bg-red-50 p-3 text-xs text-red-900">
        <div className="font-semibold mb-2">ShipmentsMain Debug</div>
        <pre className="whitespace-pre-wrap break-words">
          {JSON.stringify(debugSnapshot, null, 2)}
        </pre>
      </div>
      {debugSnapshot && (
        <div className="mx-4 rounded-lg border border-amber-300 bg-amber-50 p-3 text-xs text-amber-900">
          <div className="font-semibold mb-2">ShipmentsMain Debug</div>
          <pre className="whitespace-pre-wrap break-words">
            {JSON.stringify(debugSnapshot, null, 2)}
          </pre>
          ...
          <div className="flex items-center gap-2">
            {selectedLines.size > 0 && (
              <div className="flex items-center gap-2 mr-2">
                ...
                <Button variant="primary" size="sm" onClick={handleCreateShipment} className="text-xs">
            ...
            <Link href="/shipments">
              <ToolbarButton variant="primary">+ 출고 입력</ToolbarButton>
            </Link>
            <Link href="/shipments_print">
              <ToolbarButton variant="secondary">오늘 출고 영수증</ToolbarButton>
            </Link>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4 px-4">
        ...
        <Card className="p-3 flex flex-col gap-1 shadow-sm border-[var(--panel-border)]">
          ...
          <span className="text-xl font-bold text-green-600">
            {summary.readyToShip + summary.waitingInbound}
          </span>
        </Card>
        <Card className="p-3 flex flex-col gap-1 shadow-sm border-[var(--panel-border)]">
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-[280px_1fr] px-4">
        ...
        <Card className="shadow-sm h-fit border-[var(--panel-border)]" id="shipments_main.filters">
          ...
          <CardBody className="grid gap-3 pt-4">
            ...
            >
              <div className="flex items-center gap-2">
                <Badge tone="neutral" className="text-[10px] px-2 py-0.5 uppercase tracking-wider">
                  status
                </Badge>
          ...
          <CardHeader className="flex items-center justify-between border-b border-[var(--panel-border)] py-2 px-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-[var(--muted)]">
                {applyFilters.length}건 중 {paginatedData.length}건 표시
              </span>
              {selectedLines.size > 0 && (
                <>
                  ...
                  <button onClick={selectAllVisible} className="text-[10px] text-primary hover:underline">
                    현재 페이지 전체 선택
                  </button>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="tabular-nums">
                            {row.sent_to_vendor_at
                              ? new Date(row.sent_to_vendor_at).toLocaleDateString("ko-KR")
                              : "-"}
                          </span>
                        </div>
                        <span className="tabular-nums">
