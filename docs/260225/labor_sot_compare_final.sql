...
-- This script does NOT change write/confirm/unconfirm logic.
...
-- Suggested post-apply checks (read-only):
-- select public.cms_fn_shipment_labor_integrity_summary_v1();
-- select count(*) as mismatches from public.v_cms_shipment_labor_sot_audit_v1 where has_mismatch;
-- select count(*) as non_100_unit
-- from public.v_cms_shipment_labor_sot_audit_v1
-- where abs(base_mod_100) > 0.0001 or abs(extra_mod_100) > 0.0001 or abs(sot_mod_100) > 0.0001;
